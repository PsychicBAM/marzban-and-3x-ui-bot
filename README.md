...
- Клиентское меню: покупка, продление, «Мой VPN», реферальная программа, инструкция по подключению, FAQ, поддержка, промокоды при оплате
- **Язык интерфейса (RU/EN):** переключатель **🌐 Язык / Language** в клиентском меню; админ-панель пока только на русском
- Админ-панель: заявки, клиенты, тарифы, статистика, настройки, рассылки, промокоды, рефералы
...
### Рассылки и акции (этап 10)

```bash
alembic upgrade head   # миграция 0007
```

**Админ:** `/admin` → `📣 Рассылки`

1. `➕ Создать рассылку` — название → текст → фото (опционально) → аудитория → предпросмотр → `🚀 Отправить`
2. `📋 История рассылок` — статус, аудитория, отправлено/всего
3. Аудитории: все, активный VPN, истёкший VPN, без VPN, истекает ≤7 дн., подписаны на акции
4. Пользователи с `promo_enabled=false` **не получают** промо-рассылки

**Клиент:** `🔔 Акции и новости` — включить/выключить промо-уведомления (по умолчанию включены).
...
Миграция **0010** добавляет `users.language_code` (`ru` по умолчанию, допустимо `en`).

| Поведение | Описание |
|---|---|
| Новый пользователь | `ru`, если Telegram `language_code` не начинается с `en`; иначе `en` |
| Переключение | **🌐 Язык / Language** → 🇷🇺 Русский / 🇬🇧 English |
| Переводы | `app/presentation/i18n.py` — функция `t(lang, key, **kwargs)` |
| Админ-панель | Остаётся на русском |

**Проверка:**

1. Новый пользователь с русским Telegram → русское меню
2. Новый пользователь с `language_code=en` → английское меню
3. Переключение RU → EN → RU сохраняет выбор в БД
4. Покупка / продление / «Мой VPN» работают с английскими кнопками
5. Админ-команды и кнопки остаются на русском

```bash
alembic upgrade head   # миграция 0010_user_language
```

### Онбординг и справка

- Улучшенное приветствие `/start` (RU/EN) с описанием **KeyGate VPN**
- **📘 Инструкция / Guide** — пошаговое подключение по устройствам (iPhone, Android, Windows, macOS, TV)
- **❓ FAQ** — частые вопросы на двух языках
- **🧾 История / History** — покупки, продления, промокоды, реферальные бонусы, действия админа (пагинация)
- **🆘 Поддержка / Support** — тикеты: создать обращение, список, ответ, закрытие
- Админ: **🩺 Статус системы** — бот, БД, панели, счётчики, последний бэкап, **ежедневный отчёт** (вкл/выкл, время, отправить сейчас)
- Админ: **🆘 Обращения** — открытые / ожидают ответа / закрытые, поиск, ответ клиенту

**Миграция:** `alembic upgrade head` → `0011_production_features` (customer_events, support_tickets, admin_report_settings)

**Cron-бэкап (ежедневно в 03:00):**
